...
let server;
let endpoint;
function requestStatus(url, method, headers, body) {
...
test("an MCP client can discover all joke tools", async () => {
    ...
    try {
        ...
        const categoryTool = result.tools.find((tool) => tool.name === "get-chuck-joke-by-category");
...
test("Host and Origin validation reject browser rebinding attempts", async () => {
    ...
    const invalidHost = await requestStatus(endpoint, "POST", {
        Accept: "application/json, text/event-stream",
        "Content-Type": "application/json",
        Host: "evil.example",
    }, body);
    assert.equal(invalidHost, 403);
    const invalidOrigin = await requestStatus(endpoint, "POST", {
        Accept: "application/json, text/event-stream",
        "Content-Type": "application/json",
